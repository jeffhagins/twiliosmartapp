...
definition(
    ...
    namespace: "",
    author: "jeff",
    description: "Twilio IVR SmartApp",
    category: "My Apps",
...
preferences {
	section("Configure Communication Settings...") {
        input "areaCode","text", title: "Area Code", required: false
        input "cellNumber","phone", title: "Cell Phone for Notifications"
        input "assignedNumber","phone",title: "Assigned Number", required: false
        input "destinationNumber","phone",title: "Assigned Number", required: false
        input "authorizedNumbers", "phone", title: "Authorized Number?", multiple: true, required: false
        input "twilioSID","text", title: "Twilio SID", required: true   
        input "twilioAuthToken","text", title: "Twilio Auth Token", required: true          
	}
}
...
// Now provision a new number in the selected area code
    ...
    try {
    	...
    	log.debug "Twilio Phone Number provisioning failed"
        throw (e)
       // log.debug "Error Body: ${e.response.data}"
    }
    ...
    // settings.assignedNumber = result.data.phone_number
    ...
    sendTwilioSms ("Hi! This is a message from your new SmartThings Phone Assistant! You can reach me at ${state.phoneNumber}")
...
// Now DE-provision the current phone number
	...
	if (state.phoneSID != "") {
        ...
        sendTwilioSms ("Twilio IVR Phone Number (${state.phoneNumber}) will be removed from your location.")
...
def installed() {
    ...
    def now = new Date()
    log.debug "Time Stamp = ${currentTime()}"

	// Normalize settings as required for Twilio

	// Generate a new OAUTH access token
    createAccessToken()
    
    log.debug "After Creating Access Token Time Stamp = ${currentTime()}"

	// provision a new sub-account
	provisionSubAccount()
    
    log.debug "After Provisioning Sub-Account Time Stamp = ${currentTime()}"
    
    // Provision New Number
    
    provisionPhoneNumber()
    
    log.debug "After Provisioning Phone Number Time Stamp = ${currentTime()}"
...
// Now provision a new number in the selected area code
    ...
    log.debug "Sending SMS using Assigned Twilio Number"
    
    def messageBody = message.encodeAsURL()
    def fromNumber = state.phoneNumber.encodeAsURL()
    def toNumber = settings.cellNumber.encodeAsURL()

    def postBody =  "From=${fromNumber}&" +
                    "To=${toNumber}&" +
                    "Body=${messageBody}"
     
    def url = "https://${state.twilioSID}:${state.twilioAuthToken}@api.twilio.com/2010-04-01/Accounts/${state.twilioSID}/SMS/Messages"
